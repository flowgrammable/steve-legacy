...
inline void
check_diagnostics() {
  steve_assert(diags_, "diagnostics not initialized");
}
...
Diagnostic_stream
operator<<(Diagnostic_stream ds, debug_node d) {
  std::stringstream ss;
  ss << d;
  ds.diag->msg += ss.str();
  return ds;
...
diagnostic_name(const Diagnostic* d) {
  ...
  steve_unreachable("unknown diagnostic kind");
}
...
void
emit(std::ostream& os, const Diagnostics& ds) {
